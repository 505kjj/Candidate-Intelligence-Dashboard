...
## 2. Top-100 composition (first cut → final submission)

| Signal (counted in the top 100) | First cut | Final |
|---|---:|---:|
| Outside India | 10 | **4** |
| Not open to work | 23 | **14** |
| Notice period > 60 days | 31 | **25** |
| Not willing to relocate (`false`) | 54 | **44** |
| GitHub missing (`-1`/null) | 16 | 20 |
| Experience outside 5–9 yrs | 14 | **9** |
| Strong experience-inconsistency / honeypot mismatch | 3 | **0** |
| `risk_level` = Low / Medium / High | 100 / 0 / 0 | **44 / 47 / 9** |
| Rows with a non-zero penalty | 0 | **68** |
| Distinct reasonings | 1 | **100 / 100** |

**Top 10 (final):** 0 outside India · 0 not open to work · 0 notice > 60 days ·
0 experience outside 5–9 years · **10 / 10 with genuine career-history evidence.**

Notes on the two counts that stay relatively high by design:

- **GitHub missing (16 → 20):** intentional. The JD values engineering proof, but a
...
- **Not willing to relocate (54 → 44):** also intentional. Among India-based
...
| **Honeypot: declared ≫ documented (≥5y gap, ≥1.6×)** | **20** | **hard** |
| Over-claim 3–5y gap / under-claim ≥2y | 6 / 4 | medium |

The strong-overclaim penalty (named constant `OVERCLAIM_STRONG_PENALTY` in
`backend/src/features.py`) was raised from 14 to 20 in the final version so the
ranker itself keeps these honeypots out of the top 100 (see §5).
...
- **Experience-inconsistency honeypots** — declared years far exceeding the duration
  ...
  career history sums to ~8.2y is penalised −20, which pushes such profiles out of
  the top 100 in the final submission (the top-100 now has **zero** strong
  experience-inconsistency honeypots).
...
| CAND_0055905 | rank 8 | rank 48, **High** | London, `willing_to_relocate: false` |
| CAND_0039754 | rank 17 | **removed** | declares 16.2y vs ~8.2y documented (penalty strengthened in final) |

### Candidate intentionally kept despite concerns

One flagged profile remains in the list, deep in the tail and clearly labelled High
risk, because its JD/skill match is genuinely strong and the challenge asks for the
*best 100* with honest tradeoffs rather than silent removal:

- **CAND_0055905 (rank 48, High)** — strong retrieval/search stack (Flipkart, Uber),
  but the reasoning states plainly: *"based in UK and not relocating to India."* The
  penalty is applied, the rank reflects the discount, and the explanation names the
  tradeoff — nothing is hidden.

**CAND_0039754 was removed from the final submission.** An earlier cut retained it at
rank 91 with a −14 honeypot penalty; the final version raises the strong-overclaim
penalty (`OVERCLAIM_STRONG_PENALTY`, 14 → 20), which pushes this 16.2y-declared /
~8.2y-documented profile out of the top 100 entirely. It is **not** present in the
final `submission.csv` / `top_candidates.json`, and the top-100 now contains **zero**
strong experience-inconsistency honeypots.
